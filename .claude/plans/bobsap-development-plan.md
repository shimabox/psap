...
- 2026-07-11 GitHub 公開 (private): https://github.com/shimabox/bobsap 。初回 CI で PHP 8.3 の composer install が失敗 → 原因は 8.4 コンテナで生成した lock。config.platform.php=8.3.0 で再生成し解決 (commit 75f7c7f)。CI 全緑（8.3/8.4/self-metrics）。教訓: lock は必ず最小サポート PHP 基準で解決する。PHP 8.5 対応（イメージ更新 + マトリクス追加）は将来候補としてユーザーに提案済み
- 2026-07-11 Phase 9 完了 (commit c8426dd) — v1.1 全フェーズ完了。Dockerfile は dev/vendor/dist/phar/dist-plantuml の5ステージ（最終ステージが dist-plantuml のため、dist のビルドには --target dist が必須。README に明記）。イメージタグは bobsap:dist / bobsap:plantuml（Makefile 準拠）。plantuml.jar 1.2026.6 / phar-composer 1.4.0 固定。export-ignore は git archive で確認済み
