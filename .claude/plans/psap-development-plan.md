...
# Docker
docker run --rm -v $PWD:/workdir ghcr.io/shimabox/psap analyze src/
```

### 出力イメージ（text）

```
psap - Stable Abstractions Principle metrics
...
- **サブエージェントへの委譲時に必ず伝えること**:
  1. このプランファイル（`.claude/plans/psap-development-plan.md`）を読むこと
  2. ソースコード内のコメントは日本語
...
- [ ] `composer.json`（PSR-4: `Psap\` → `src/`、`Psap\Tests\` → `tests/`。scripts: `test`, `stan`, `cs`（dry-run チェック）, `cs:fix`（自動整形））
...
- [ ] ドッグフーディング: psap 自身を psap で計測し、結果を README に載せる
...
- [ ] コンポーネント間依存グラフの導出を PlantUmlReporter の private から共有クラスへ抽出（`Psap\Component\DependencyGraph`: ノード=コンポーネント名、エッジ=依存。導出ロジックは既存と同一）
...
- [ ] テスト: docblock 内の短縮名が use 文・現在名前空間で FQCN 解決されること（php-parser の NameContext を活用。php-class-diagram は自前解決だが、psap は NameResolver の文脈を使う方が素直）
...
- [ ] セルフ計測ジョブ: psap で src/ を計測し、text + mermaid + plantuml 出力を Artifact にアップロード
...
- 設定ファイル（`.psap.json` 等）。v1 は CLI オプションのみ
- ディレクトリベースのコンポーネント分類
- HTML レポート
...
- 2026-07-11 GitHub 公開 (private): https://github.com/shimabox/psap 。初回 CI で PHP 8.3 の composer install が失敗 → 原因は 8.4 コンテナで生成した lock。config.platform.php=8.3.0 で再生成し解決 (commit 75f7c7f)。CI 全緑（8.3/8.4/self-metrics）。教訓: lock は必ず最小サポート PHP 基準で解決する。PHP 8.5 対応（イメージ更新 + マトリクス追加）は将来候補としてユーザーに提案済み
- 2026-07-11 Phase 9 完了 (commit c8426dd) — v1.1 全フェーズ完了。Dockerfile は dev/vendor/dist/phar/dist-plantuml の5ステージ（最終ステージが dist-plantuml のため、dist のビルドには --target dist が必須。README に明記）。イメージタグは psap:dist / psap:plantuml（Makefile 準拠）。plantuml.jar 1.2026.6 / phar-composer 1.4.0 固定。export-ignore は git archive で確認済み
- 2026-07-11 Phase 7 完了 (commit c66bbe0) — docblock 解析は DocblockTypeExtractor（phpstan/phpdoc-parser ^2）。デフォルト有効・--no-docblock でオフ。疑似型はリストベース判定。Collection<X> は外側クラスも依存に数える。壊れた docblock は無視
...
- 2026-07-11 Phase 5 完了（未コミット） — `docker/Dockerfile` を dev/vendor/dist の3ステージに分割（`compose.yaml` は `target: dev` を明示指定して既存の開発フローを維持）。dist イメージは `ENTRYPOINT ["php", "/app/bin/psap"]` で `/workdir` を作業ディレクトリに、ツール本体は `/app` に配置。README・LICENSE・ドッグフーディング結果を追加。Composer 経由インストールは Packagist 未公開のため git リポジトリ指定の例のみ記載（判断: `composer require --dev shimabox/psap` は将来対応として明記）。実 OSS 動作確認は `thephpleague/csv`（123ファイル）に対して depth=2/3 双方でクラッシュなく完走（exit 0）。ghcr.io への実際の publish は行っていない（プランの記述はビルド例として README に残すか検討したが、混乱を避けローカルビルド手順のみ記載）
- 2026-07-11 Phase 8 完了（未コミット） — `.github/workflows/ci.yml` を新規作成。`checks` ジョブは Docker ではなく shivammathur/setup-php（PHP 8.3/8.4 マトリクス、actions/cache で vendor キャッシュ）を採用（Actions 上では Docker ビルドより高速なため）。`self-metrics` ジョブは `checks` 成功後に psap 自身の `src/` を計測し、text 出力で `--fail-on-cycle` を兼ねる（別実行にせず1ステップに統合）。mermaid/plantuml/json も `--output` で `psap-report/` に出力し actions/upload-artifact@v4 でアップロード、text 表と mermaid 図（```mermaid フェンス）を `$GITHUB_STEP_SUMMARY` に書き込む。actionlint（rhysd/actionlint Docker）で構文確認済み（初回 shellcheck SC2129 警告は `{ } >> file` 形式に直して解消、最終 exit 0）。実際の GitHub 上の実行確認は push 後になる
