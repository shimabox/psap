...
<head>
  ...
  <title>psap — Architecture portal</title>
...
<body>
  <main class="shell">
    <header class="masthead">
      <div>
        <p class="eyebrow" data-i18n="eyebrow">psap / architecture portal</p>
        <h1 data-i18n="headline">Architecture Portal</h1>
  ...
  <script>
    (() => {
      ...
      const messages = {
        en: {
          documentTitle: 'psap — Architecture portal',
          eyebrow: 'psap / architecture portal',
          headline: 'Architecture Portal',
        ...
        ja: {
          documentTitle: 'psap — アーキテクチャポータル',
          eyebrow: 'psap / architecture portal',
          headline: 'アーキテクチャポータル',
      ...
      function applyLanguage() {
        ...
        // Re-apply dynamic diagram fallback text after a language change.
        const fallback = document.querySelector('#diagram-flowchart .fallback');
        if (fallback) {
          fallback.textContent = t('flowchartSkipped', { edges: portal.flowchart.edgeCount, max: portal.flowchart.maxEdges });
        }
      }
      ...
      document.querySelectorAll('[data-copy]').forEach((button) => {
        button.addEventListener('click', async () => {
          ...
          try {
            ...
            range.selectNodeContents(document.getElementById('source-' + button.dataset.copy));
      ...
      function renderDiagrams() {
        ...
        if (!portal.flowchart.renderable) {
          ...
          notice.textContent = t('flowchartSkipped', { edges: portal.flowchart.edgeCount, max: portal.flowchart.maxEdges });
          flowchartContainer.replaceChildren(notice);
          return;
        }
      ...
      async function drawDiagram(containerId, renderId, source) {
        ...
        try {
          ...
          const notice = document.createElement('p');
          notice.className = 'fallback';
          notice.textContent = t('diagramError');
