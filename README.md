...
`Analysis coverage`には、発見、選択、解析、除外、スキップしたPHPファイル数が表示されます。スキップがある場合はWarningsを確認してから設計判断に使ってください。カバレッジ100%は選択したPHPファイルをすべて処理できたことを示しますが、動的参照や設定ファイルだけに書かれた依存まで検出したことを意味しません。
